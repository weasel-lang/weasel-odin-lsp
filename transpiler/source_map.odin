/*
	Source map: correspondence between byte ranges in the generated host output
	and their originating `.weasel` source spans.

	The transpiler returns a Source_Map alongside the generated host string.
	Entries are emitted in arbitrary order and sorted by host_start.offset at
	...
	between Weasel and host positions lives in the LSP layer (package lsp),
...
// host_start.offset.  The transpiler appends entries during emission and
...
// token-internal positions (e.g. the procedure name inside a wider Host_Text
...
// _sort_entries orders entries by host_start.offset.  Called by transpile()
